...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (!secret || !supabase) {
    return res.status(500).json({
      ok: false,
      error: 'Missing secure panel configuration',
      missingEnv: true,
    });
  }
  ...
  if (publicError) {
    console.error('Panel catalog data failed:', publicError);
    return res.status(500).json({
      ok: false,
      error: 'Could not load panel catalog data',
      code: publicError.code || null,
    });
  }
  ...
  if (panel === 'delivery') {
    ...
    const customersResult = phones.length
      ? await supabase
          .from('customers')
          .select(
            'id,phone,name,avatar_url,lat,lng,reference,delivery_addresses,selected_delivery_address_id'
          )
          .in('phone', phones)
      : { data: [], error: null };
