...
const getPendingPaymentText = (_order: Order) => {
...
export default function OrderTracking({
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <section className="relative z-10 w-full sm:max-w-md max-h-[82dvh] bg-white rounded-t-[36px] sm:rounded-[36px] shadow-2xl animate-in slide-in-from-bottom-8 sm:zoom-in-95 duration-300 border border-white/80 overflow-hidden flex flex-col">
        <header className={`relative overflow-hidden bg-gradient-to-br ${heroTone.hero} text-white px-5 pt-[calc(env(safe-area-inset-top)+12px)] sm:pt-4 pb-3 flex-shrink-0`}>
          ...
          <div className="relative pr-12">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-[20px] flex items-center justify-center shadow-xl border bg-white/20 text-white border-white/25">
                <HeaderIcon size={24} />
              </div>

              <div className="min-w-0 flex-1">
                <p className="text-[9px] font-black uppercase tracking-[0.24em] text-white/75">
                  Rastreo Pollazo
                </p>

                <h2 className="text-lg font-black uppercase italic leading-none mt-1.5">
                  {hasActiveOrder ? getStatusTitle(currentStatus) : 'Rastreo en vivo'}
                </h2>

                <p className="text-[11px] font-bold text-white/80 leading-relaxed mt-1.5 line-clamp-1">
        ...
        <div className="flex-1 overflow-y-auto px-4 pt-3 pb-[calc(env(safe-area-inset-bottom)+12px)] space-y-2.5 bg-gradient-to-b from-orange-50/45 via-white to-white">
          {renderTrackingNotice(false)}

          {hasActiveOrder && currentStatus && activeOrder ? (
            <>
              {currentStatus !== 'Cancelado' && (
                <section className="bg-white border border-orange-100 rounded-[28px] p-3 shadow-sm overflow-hidden">
              ...
              {currentStatus !== 'Por Confirmar' && (
                <section className={`rounded-[26px] border p-3 shadow-sm ${heroTone.message}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold leading-relaxed mt-1 opacity-80">
              ...
              {paymentTone && currentStatus !== 'Por Confirmar' && (
                <section className={`rounded-[26px] border p-3 shadow-sm ${paymentTone.wrapper}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={`text-[9px] font-bold leading-relaxed mt-1 ${paymentTone.text}`}>
                        {getPaymentMethodLabel(activeOrder.payment_method)}
                      </p>
                    </div>
                  </div>
                </section>
              ...
              {estimate && (
                <section className="bg-white border border-orange-100 rounded-[28px] p-3 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
