...
export default function AdminPosSmartLauncher() {
  ...
  const closeDifference = useMemo(() => {
    if (!register) return 0;
    return parseMoney(closingCash || expectedCash) - expectedCash;
  ...
  const availableProducts = useMemo(() => {
    return products
      ...
      .filter(product => product.show_in_pos !== false)
  ...
  const filteredProducts = useMemo(() => {
    ...
    return list.slice(0, clean ? 16 : 10);
                  ...
                  <section className="space-y-4">
                    <div className="rounded-[28px] bg-white border border-gray-100 p-4 shadow-sm">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                        <div>
                          ...
                          <p className="text-xs font-bold text-gray-400">Inicial ${money(register.openingBalance)} · Efectivo vendido ${money(register.expectedCashSales)} · Caja {register.id.slice(0, 8)}</p>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-3">
                        ...
                        <div className="rounded-2xl bg-green-50 border border-green-100 p-3"><p className="text-[8px] font-black uppercase text-green-600">Efectivo</p><p className="text-sm font-black text-gray-900">${money(register.expectedCashSales)}</p></div>
                    ...
                    {loadingProducts ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        {filteredProducts.length === 0 && <div className="sm:col-span-2 rounded-3xl border border-dashed border-gray-200 bg-white p-8 text-center text-gray-400"><PackageSearch size={32} className="mx-auto mb-2 text-orange-300" /><p className="text-xs font-black uppercase">No encontré productos marcados para caja</p></div>}
                      </div>
                    )}
                    ...
                    <section className="rounded-[28px] bg-white border border-gray-100 p-4 shadow-sm">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-gray-400 mt-1">Resumen rápido desde que abriste caja.</p>
                        </div>
                    ...
                    <section className="rounded-[28px] bg-white border border-red-100 p-4 shadow-sm">
                      ...
                      <p className="text-[10px] font-bold text-gray-400 mt-1">Al terminar el día o cambiar de cajero, cuenta el efectivo físico.</p>
                      ...
                      {Math.abs(closeDifference) > 0.01 && (
                        <div className={`mb-3 rounded-2xl border p-3 flex items-start gap-2 ${closeDifference < 0 ? 'bg-red-50 border-red-100 text-red-600' : 'bg-yellow-50 border-yellow-100 text-yellow-700'}`}>
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-[9px] font-bold opacity-80">Revisa antes de cerrar. Si cierras así, aparecerá en Reportes POS.</p>
                          </div>
                        </div>
                      )}
