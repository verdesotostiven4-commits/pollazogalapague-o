...
export default function AppHeader({ screen, onNavigate, onOpenProfile, customerAvatar }: Props) {
  // ✅ Usamos cartCount para que la burbuja marque 1, 2, 3...
  ...
  return (
    <header className="flex-shrink-0 safe-area-top bg-white/80 backdrop-blur-md border-b border-orange-50 shadow-sm z-40 sticky top-0">
      <div className="flex items-center justify-between px-4 h-14 relative">
        {isHome ? (
          <div onClick={handleRefresh} className="flex items-center gap-2 cursor-pointer active:scale-95 transition-transform">
            <img src={LOGO_URL} alt="logo" className="h-10 w-10 object-contain drop-shadow-sm transition-opacity duration-300" />
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => onNavigate('ranking')} className={`w-9 h-9 flex items-center justify-center rounded-xl transition-all active:scale-90 ${screen === 'ranking' ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-gray-100 text-gray-400'}`}>
            <BarChart2 size={18} />
          </button>

          <button onClick={onOpenProfile} className="w-9 h-9 rounded-xl bg-orange-50 overflow-hidden border border-orange-100 flex items-center justify-center active:scale-90 transition-transform">
            {customerAvatar ? <img src={customerAvatar} className="w-full h-full object-cover" alt="perfil" /> : <User size={18} className="text-orange-500" />}
          </button>

          <button ref={cartBtnRef} onClick={() => onNavigate('cart')} className={`relative w-9 h-9 flex items-center justify-center rounded-xl transition-all active:scale-90 ${screen === 'cart' ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-orange-50 text-orange-500'} ${cartPop ? 'scale-110' : ''}`}>
