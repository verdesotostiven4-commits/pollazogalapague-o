...
const slug = (value: string) => normalized(value).replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || `producto-${Date.now()}`;
const hasImage = (product: CatalogProduct) => String(product.image || '').trim().length > 8 && !String(product.image || '').includes('logo-final');
...
export default function AdminCatalogMasterLauncher() {
  ...
  const toggleAvailability = async () => {
    ...
    if (toggleError) {
      ...
      setMessage(next ? 'Producto visible nuevamente.' : 'Producto oculto/agotado.');
      await loadProducts();
    }
  };
  ...
  return (
    <>
      {!open && (
        <button type="button" onClick={() => setOpen(true)} className="fixed bottom-44 right-5 z-[9995] flex items-center gap-2 rounded-[24px] bg-white px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-950 shadow-2xl shadow-orange-100/70 ring-4 ring-white border border-orange-100 active:scale-95 transition-transform">
          ...
          <div className="mx-auto max-w-6xl rounded-[34px] bg-gray-50 shadow-2xl overflow-hidden border border-white/40">
            <header className="bg-gradient-to-r from-slate-950 via-slate-900 to-orange-950 text-white p-4 sm:p-5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="h-12 w-12 rounded-2xl bg-orange-500 flex items-center justify-center shadow-lg shadow-orange-500/30"><ClipboardList size={24} /></div>
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-[0.25em] text-orange-300">Automatización</p>
                  <h2 className="text-xl sm:text-2xl font-black uppercase italic leading-none truncate">Catálogo Maestro</h2>
                </div>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="h-11 w-11 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center active:scale-90 transition-transform"><X size={20} /></button>
            </header>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-4">
                <section className="rounded-[32px] bg-white border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-4 border-b border-gray-100 flex items-center justify-between gap-3">
                    <div><p className="text-xs font-black uppercase text-gray-900">Productos</p><p className="text-[10px] font-bold text-gray-400">Busca y selecciona para corregir.</p></div>
                    <button type="button" onClick={loadProducts} disabled={loading} className="h-11 w-11 rounded-2xl bg-slate-950 text-white flex items-center justify-center disabled:opacity-60">{loading ? <Loader2 size={17} className="animate-spin" /> : <RefreshCw size={17} />}</button>
                  </div>
                  <div className="p-4 border-b border-gray-100"><div className="relative"><Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar nombre, categoría o código..." className="w-full rounded-2xl border border-gray-100 bg-gray-50 py-4 pl-11 pr-4 text-sm font-bold outline-none focus:border-orange-300" /></div></div>
                ...
                <section className="rounded-[32px] bg-white border border-gray-100 shadow-sm p-4 space-y-4">
                  <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase text-gray-900">Ficha del producto</p><p className="text-[10px] font-bold text-gray-400">Completa y guarda una sola vez.</p></div><button type="button" onClick={() => { setSelectedId(null); setDraft(blankDraft()); }} className="rounded-2xl bg-orange-500 px-4 py-3 text-[10px] font-black uppercase text-white active:scale-95">Nuevo</button></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })} placeholder="Nombre" className="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none" />
                    <input value={draft.price} disabled={draft.is_variable} onChange={e => setDraft({ ...draft, price: e.target.value })} placeholder="Precio Ej: 2.50" className="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none disabled:opacity-40" />
                    <select value={draft.category} onChange={e => { const category = e.target.value as Category; setDraft({ ...draft, category, subcategory: SUBS[category]?.[0] || '' }); }} className="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none">{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
                    <select value={draft.subcategory} onChange={e => setDraft({ ...draft, subcategory: e.target.value })} className="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none">{(SUBS[draft.category] || []).map(s => <option key={s} value={s}>{s}</option>)}</select>
                    <input value={draft.unit} onChange={e => setDraft({ ...draft, unit: e.target.value })} placeholder="Unidad" className="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none" />
                    <input value={draft.barcode} onChange={e => setDraft({ ...draft, barcode: e.target.value })} placeholder="Código interno / barras" className="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none" />
                    <input value={draft.image} onChange={e => setDraft({ ...draft, image: e.target.value })} placeholder="Link imagen" className="md:col-span-2 rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none" />
                    <textarea value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })} placeholder="Descripción corta" className="md:col-span-2 rounded-2xl bg-gray-50 border border-gray-100 px-4 py-4 text-sm font-bold outline-none min-h-[88px] resize-none" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 rounded-[26px] bg-orange-50 border border-orange-100 p-3"><input value={draft.cost_price} onChange={e => setDraft({ ...draft, cost_price: e.target.value })} placeholder="Costo" className="rounded-2xl bg-white border border-orange-100 px-4 py-4 text-sm font-bold outline-none" /><input value={draft.current_stock} onChange={e => setDraft({ ...draft, current_stock: e.target.value })} placeholder="Stock actual" className="rounded-2xl bg-white border border-orange-100 px-4 py-4 text-sm font-bold outline-none" /><input value={draft.stock_minimum} onChange={e => setDraft({ ...draft, stock_minimum: e.target.value })} placeholder="Stock mínimo" className="rounded-2xl bg-white border border-orange-100 px-4 py-4 text-sm font-bold outline-none" /></div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><button type="button" onClick={() => setDraft({ ...draft, available: !draft.available })} className={`rounded-2xl border p-4 text-left ${draft.available ? 'bg-green-50 text-green-700 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>{draft.available ? <Eye size={18} /> : <EyeOff size={18} />}<p className="mt-2 text-[10px] font-black uppercase">{draft.available ? 'Visible' : 'Oculto'}</p></button><button type="button" onClick={() => setDraft({ ...draft, is_variable: !draft.is_variable })} className="rounded-2xl border p-4 text-left bg-gray-50 text-gray-600 border-gray-100"><Sparkles size={18} /><p className="mt-2 text-[10px] font-black uppercase">{draft.is_variable ? 'Variable' : 'Precio fijo'}</p></button><button type="button" onClick={() => setDraft({ ...draft, track_stock: !draft.track_stock })} className="rounded-2xl border p-4 text-left bg-gray-50 text-gray-600 border-gray-100"><PackageCheck size={18} /><p className="mt-2 text-[10px] font-black uppercase">{draft.track_stock ? 'Stock activo' : 'Sin stock'}</p></button></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{selected && <button type="button" onClick={toggleAvailability} className="rounded-2xl bg-gray-100 px-5 py-4 text-[10px] font-black uppercase text-gray-600 active:scale-95">{selected.available === false ? 'Mostrar producto' : 'Ocultar producto'}</button>}<button type="button" onClick={save} disabled={saving} className="rounded-2xl bg-slate-950 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-white active:scale-95 disabled:opacity-60 flex items-center justify-center gap-2"><Save size={16} />{saving ? 'Guardando...' : 'Guardar y sincronizar'}</button></div>
                </section>
              </div>
            </main>
          </div>
