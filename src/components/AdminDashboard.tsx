  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6 text-white text-center">
      <div className="w-full max-w-xs space-y-8 animate-in fade-in zoom-in duration-500">
        ...
        <h1 className="text-xl font-black italic uppercase tracking-widest leading-none">Admin VIP</h1>
...
export default function AdminDashboard() {
  ...
  if (!context) return <div className="min-h-screen flex items-center justify-center bg-gray-950 text-orange-500 font-black">CARGANDO...</div>;
  
  const { customers = [], orders = [], seasons = [], extraSettings = {}, updateExtraSettings, finalizeSeason, deleteSeason, toggleSeasonVisibility, updateSeasonWinners, addCustomerPoints, updateOrderStatus, refreshData, loading } = context;

  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-950 text-orange-500 font-black animate-pulse">SINCRONIZANDO...</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans text-slate-900">
      <header className="sticky top-0 z-40 bg-white border-b p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2"><img src={extraSettings?.logo_url || '/logo-final.png'} className="w-8 h-8 object-contain"/><p className="font-black text-[10px] uppercase tracking-tighter">Admin VIP</p></div>
               ...
               return (
                 <div key={o.id} className="bg-white rounded-[32px] border p-5 space-y-4 shadow-sm">
                    <div className="flex justify-between items-center">
                       ...
                       <div className="text-right leading-none"><p className="font-black text-orange-600 text-sm italic">${Number(o.total || 0).toFixed(2)}</p><p className="text-[7px] font-black text-gray-300 uppercase mt-1">{o.status}</p></div>
                    </div>
