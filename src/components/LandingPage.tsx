...
export default function LandingPage({ onInstall, canInstall, onContinueWeb }: Props) {
  ...
  useEffect(() => {
    ...
    // 3. Capturar evento de instalación nativa
  ...
  const handleInstallClick = async () => {
    ...
    setInstallMessage('Abre el menú de tu navegador e instala la App manualmente.');
  };
  ...
  return (
    <div className="bg-white text-gray-950 overflow-x-hidden min-h-screen" style={{ '--pollazo-primary': primaryColor } as CSSProperties}>
      <style>
        {`
          ...
          .pollazo-logo-float { animation: pollazoFloat 5s ease-in-out infinite; }
      ...
      {/* SECCIÓN HERO / SPLASH (Solo se muestra si NO es Standalone) */}
      {!isStandalone && (
          ...
          <div className="relative z-10 max-w-md mx-auto space-y-10" style={{ opacity: visible ? 1 : 0, transition: 'opacity 0.8s ease' }}>
            ...
            <div className="w-full max-w-xs mx-auto space-y-4">
              ...
              <button 
                ...
                className="text-xs font-bold text-white/75 mx-auto active:opacity-50 flex items-center gap-2 justify-center"
              >
                <Globe size={14} /> Explorar Catálogo
              </button>
              {installMessage && <p className="text-white/75 text-[10px] font-bold uppercase">{installMessage}</p>}
            </div>
          </div>
        </section>
      )}
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-orange-100 h-16 flex items-center justify-between px-5">
        ...
        <button className="w-10 h-10 rounded-2xl bg-orange-50 flex items-center justify-center">
      ...
      <main className="px-5 py-10 space-y-16">
        ...
        <section className="rounded-[40px] p-8 text-white shadow-2xl bg-orange-500 relative overflow-hidden">
          <div className="relative z-10 space-y-4">
            ...
            <p className="text-sm opacity-90 font-medium">Pollo fresco enfundado y atención premium para las familias de Puerto Ayora.</p>
          </div>
        ...
        <section className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3 h-64">
            ...
            <div className="space-y-3">
              <div className="h-[120px] rounded-[24px] overflow-hidden shadow-md">
                <img src="https://images.pexels.com/photos/616354/pexels-photo-616354.jpeg" className="w-full h-full object-cover" alt="" />
              </div>
              <div className="h-[120px] rounded-[24px] overflow-hidden shadow-md">
                <img src="https://images.pexels.com/photos/3962285/pexels-photo-3962285.jpeg" className="w-full h-full object-cover" alt="" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-gray-950 rounded-[40px] p-8 text-white space-y-6">
          ...
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="¿Algún comentario adicional?" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm outline-none focus:ring-2 focus:ring-orange-500" rows={3} />
          <button disabled={userRating === 0} onClick={handleSendReview} className={`w-full py-4 rounded-2xl font-black transition-all ${userRating > 0 ? 'bg-orange-500 text-white shadow-lg active:scale-95' : 'bg-white/5 text-gray-800 cursor-not-allowed'}`}>Enviar Opinión</button>
        </section>
        ...
        <section className="bg-white p-6 rounded-[40px] border border-orange-100 shadow-xl space-y-4">
          ...
          <p className="text-xs font-semibold text-gray-500 leading-relaxed">Encuentra los mejores pollos de la isla y productos para tu hogar.</p>
      ...
      {showIOSModal && (
        <div className="fixed inset-0 z- bg-black/60 backdrop-blur-sm flex items-end justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-[32px] p-7 shadow-2xl relative">
            <button onClick={() => setShowIOSModal(false)} className="absolute top-4 right-4 w-10 h-10 rounded-2xl bg-gray-100 flex items-center justify-center"><X size={18} /></button>
            <div className="space-y-6">
              ...
              <button onClick={() => setShowIOSModal(false)} className="w-full py-4 rounded-2xl bg-orange-500 text-white font-black">Entendido</button>
            </div>
          </div>
        </div>
      )}
