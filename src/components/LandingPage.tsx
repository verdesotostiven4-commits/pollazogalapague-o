...
export default function LandingPage({ onInstall, canInstall, onContinueWeb }: Props) {
  ...
  return (
    <div className="bg-white text-gray-950 overflow-x-hidden min-h-screen" style={{ '--pollazo-primary': primaryColor } as CSSProperties}>
      ...
      {/* HERO / SPLASH (Se oculta si ya es App) */}
      {!isStandalone && (
      ...
      <main className="px-5 py-10 space-y-16">
        <section className="rounded-[40px] p-8 text-white shadow-2xl bg-orange-500 relative overflow-hidden">
          <div className="relative z-10 space-y-4">
            ...
            <p className="text-sm opacity-90 font-medium">Pollo fresco enfundado y atención premium en Puerto Ayora.</p>
          </div>
        ...
        <section className="bg-gray-950 rounded-[40px] p-8 text-white space-y-6">
          ...
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="¿Algún comentario adicional?" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white outline-none focus:ring-2 focus:ring-orange-500" rows={3} />
          <button disabled={userRating === 0} onClick={handleSendReview} className={`w-full py-4 rounded-2xl font-black transition-all ${userRating > 0 ? 'bg-orange-500 text-white shadow-lg' : 'bg-white/5 text-gray-800 cursor-not-allowed'}`}>Enviar Opinión</button>
        </section>
