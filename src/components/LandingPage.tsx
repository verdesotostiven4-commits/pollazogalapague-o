...
const LOGO_URL = "https://blogger.googleusercontent.com/img/a/AVvXsEin3pN4YDaHP3IxmNrtpbD2swEb9qpEJOsOmbbbtAmlaSgSicNgZbB9jYTdfdX4oiDOBORD4h5oDSRlFbzw6-3B6c2sFH7s3T0tla5kFjCe6treln_EPQ5a2i7V-ghUqJyTeVztj1ORThqO-G-eqO1eyDxo3MsEsoiBW60fatCa7SNeVHtJd-a3vLrjhtg";
...
export default function LandingPage({ onInstall, canInstall, onContinueWeb }: Props) {
  ...
  const logoUrl = extraSettings?.logo_url || LOGO_URL;
  ...
  return (
    <div className="bg-white text-gray-950 overflow-x-hidden min-h-screen font-sans" style={{ '--pollazo-primary': primaryColor } as CSSProperties}>
      ...
      {/* HERO / SPLASH - Solo visible en Web */}
      {!isStandalone && (
        <section className="relative min-h-screen flex flex-col items-center justify-center px-6 text-center bg-gradient-to-b from-orange-500 via-orange-400 to-orange-300">
          ...
          <div className="relative z-10 max-w-md mx-auto space-y-10" style={{ opacity: visible ? 1 : 0, transition: 'opacity 0.8s ease' }}>
            ...
            <div className="w-full max-w-xs mx-auto space-y-4">
              ...
              <button onClick={() => window.scrollTo({ top: window.innerHeight, behavior: 'smooth' })} className="text-xs font-bold text-white/75 mx-auto flex items-center gap-2 justify-center">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-orange-100 h-16 flex items-center justify-between px-5">
        ...
        <button className="w-10 h-10 rounded-2xl bg-orange-50 flex items-center justify-center"><Bell size={18} className="text-orange-500" /></button>
      </header>
      ...
      <main className="px-5 py-10 space-y-16">
        ...
        {/* RESEÑAS - EL ARRAY ESTÁ AQUÍ */}
        <section className="bg-gray-950 rounded-[40px] p-8 text-white space-y-6">
          <div className="text-center space-y-2">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-400">RESEÑAS</p>
            <h2 className="text-xl font-black">¿Cómo fue tu experiencia?</h2>
          </div>
          ...
          <button 
            ...
            className={`w-full py-4 rounded-2xl font-black transition-all ${userRating > 0 ? 'bg-orange-500 text-white shadow-lg' : 'bg-white/5 text-gray-800 cursor-not-allowed'}`}
          >
            Enviar Opinión
          </button>
        </section>
