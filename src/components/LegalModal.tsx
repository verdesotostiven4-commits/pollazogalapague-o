...
function getLegalSections(language: LanguageCode) {
  return language === 'es' ? esSections : enSections;
}
  ...
  return (
    <section className="bg-white border border-orange-100/70 rounded-[30px] p-4 shadow-sm">
      <div className="flex items-start gap-3 mb-3">
        ...
        <h3 className="text-[11px] font-black text-gray-950 uppercase tracking-widest leading-tight pt-1">
          {title}
        </h3>
      </div>
...
function MiniLegalCard({ icon, title, text }: { icon: ReactNode; title: string; text: string }) {
  return (
    <div className="bg-white/80 border border-white rounded-[24px] p-3 shadow-sm">
      ...
      <p className="text-[9px] font-black text-gray-900 uppercase leading-tight">{title}</p>
  ...
  return (
    <div className="bg-orange-50/70 border border-orange-100 rounded-[24px] p-3 flex items-start gap-3">
      ...
      <div className="min-w-0">
        <p className="text-[10px] font-black text-gray-900 uppercase leading-tight">{title}</p>
        <p className="text-[10px] font-bold text-gray-500 leading-relaxed mt-1">{text}</p>
      </div>
    </div>
  );
}

export default function LegalModal({ isOpen, onClose, mode = 'auto' }: Props) {
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      scrollIdleTimerRef.current = window.setTimeout(() => setIsScrolling(false), 760);
    };
  ...
  const handlePrimaryAction = () => {
    if (isRequired && !hasUnlockedAccept) return;
  ...
  const modalTitle = isRequired ? t('legal.required.title') : t('legal.read.title');
  const modalSubtitle = isRequired ? t('legal.required.subtitle') : t('legal.read.subtitle');
  ...
  return (
    <div className="fixed inset-0 z-[12000] flex items-end sm:items-center justify-center bg-orange-950/20 p-0 sm:p-4">
      <style>{`
        ...
        .pollazo-legal-arrow { animation: pollazoLegalArrowBounce 1.05s ease-in-out infinite; }
        .pollazo-legal-scroll { scrollbar-width: none; overscroll-behavior: contain; -webkit-overflow-scrolling: touch; }
        .pollazo-legal-scroll::-webkit-scrollbar { width: 7px; }
        .pollazo-legal-scroll::-webkit-scrollbar-thumb,
        .pollazo-legal-scroll::-webkit-scrollbar-track { background: transparent; border-radius: 999px; }
        .pollazo-legal-scroll.is-scrolling { scrollbar-width: thin; scrollbar-color: #fb923c #fff7ed; }
        .pollazo-legal-scroll.is-scrolling::-webkit-scrollbar-thumb { background: #fb923c; }
        .pollazo-legal-scroll.is-scrolling::-webkit-scrollbar-track { background: #fff7ed; }
      `}</style>

      {isReadOnly ? (
        <button type="button" aria-label={t('common.close')} onClick={onClose} className="absolute inset-0 bg-orange-950/10" />
      ) : (
      ...
      <section className="relative w-full sm:max-w-lg h-[94dvh] sm:h-[90vh] bg-gradient-to-b from-orange-50 via-white to-white rounded-t-[40px] sm:rounded-[40px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-300 border border-white/80 flex flex-col">
        <header className="flex-shrink-0 bg-white/95 border-b border-orange-100 px-5 pt-[calc(env(safe-area-inset-top)+16px)] sm:pt-5 pb-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h2 className="text-base font-black text-gray-950 uppercase italic leading-tight truncate">{modalTitle}</h2>
                <p className="text-[9px] font-black text-orange-500 uppercase tracking-[0.22em] mt-1 truncate">{modalSubtitle}</p>
              </div>
            </div>
          ...
          <section className="relative overflow-hidden bg-gradient-to-br from-white via-orange-50 to-yellow-50 border border-orange-100 rounded-[36px] p-5 shadow-sm">
            ...
            <div className="relative">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] font-black text-orange-500 uppercase tracking-[0.24em]">{t('legal.hero.kicker')}</p>
                  <h3 className="text-2xl font-black text-gray-950 uppercase italic leading-none mt-2">{t('legal.hero.title')}</h3>
                  <p className="text-[11px] font-bold text-gray-500 leading-relaxed mt-3">{t('legal.hero.text')}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2 mt-5">
                <MiniLegalCard icon={<FileText size={18} />} title={t('legal.category.orders')} text={t('legal.category.orders_text')} />
                <MiniLegalCard icon={<CreditCard size={18} />} title={t('legal.category.payments')} text={t('legal.category.payments_text')} />
                <MiniLegalCard icon={<Lock size={18} />} title={t('legal.category.data')} text={t('legal.category.data_text')} />
              </div>
            </div>
          </section>

          {isRequired && (
            <div className="bg-orange-50 border border-orange-100 rounded-[24px] p-3 flex items-start gap-3">
              <ShieldCheck size={18} className="text-orange-500 flex-shrink-0 mt-0.5" />
              <p className="text-[10px] font-black text-orange-700 uppercase leading-relaxed">{t('legal.required.notice')}</p>
            </div>
          )}

          <div className="px-1">
            <p className="text-[10px] font-black text-orange-500 uppercase tracking-[0.24em]">{t('legal.info_title')}</p>
          </div>

          {legalSections.map((section, index) => (
            <Section key={section.title} icon={SECTION_ICONS[index] || <FileText size={20} />} title={section.title}>
              {section.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}
            </Section>
          ))}

          <div className="px-1 pt-1">
            <p className="text-[10px] font-black text-orange-500 uppercase tracking-[0.24em]">{t('legal.help_title')}</p>
          </div>

          <section className="bg-white border border-orange-100 rounded-[32px] p-4 shadow-sm space-y-3">
            <HelpItem icon={<ClipboardList size={18} />} title={t('legal.help.order')} text={t('legal.help.order_text')} />
            <HelpItem icon={<CreditCard size={18} />} title={t('legal.help.payment')} text={t('legal.help.payment_text')} />
            <HelpItem icon={<MapPin size={18} />} title={t('legal.help.location')} text={t('legal.help.location_text')} />
            <HelpItem icon={<BellRing size={18} />} title={t('legal.help.notifications')} text={t('legal.help.notifications_text')} />
            <HelpItem icon={<Smartphone size={18} />} title={t('legal.help.security')} text={t('legal.help.security_text')} />
            <HelpItem icon={<UserCheck size={18} />} title={t('legal.help.data')} text={t('legal.help.data_text')} />
            <a
        ...
        {isRequired && showScrollArrow && (
          <button
            type="button"
            onClick={scrollToEnd}
            className="absolute bottom-[calc(env(safe-area-inset-bottom)+24px)] left-1/2 z-20 -translate-x-1/2 text-orange-500 drop-shadow-[0_2px_6px_rgba(124,45,18,0.35)] active:scale-90 transition-transform"
        ...
        {(!isRequired || hasUnlockedAccept) && (
          <footer className="flex-shrink-0 bg-white/95 border-t border-orange-100 px-5 pt-3 pb-[calc(env(safe-area-inset-bottom)+14px)]">
            <p className="text-[9px] font-bold text-gray-400 leading-relaxed text-center mb-3">
              {isRequired ? t('legal.footer_required') : t('legal.footer_read')}
            </p>
            <button
              type="button"
