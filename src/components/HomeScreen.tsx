...
function RevealOnScroll({ children, delay = 0 }: { children: React.ReactNode, delay?: number }) {
  ...
  return (
    <div ref={ref} className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${delay}ms` }}>
      {children}
    </div>
  );
}
...
export default function HomeScreen({ onNavigate, onNavigateToCategory }: Props) {
  const user = useUser();
  const { products = [] } = useAdmin(); // 🛡️ Default a array vacío para evitar crash en .filter
  const [activeIndex, setActiveIndex] = useState(0);
  const scrollRef = useRef<HTMLDivElement>(null);

  // 🛡️ Filtro seguro de productos
  ...
  const scrollTo = (idx: number) => {
    if (scrollRef.current) scrollRef.current.scrollTo({ left: idx * scrollRef.current.offsetWidth, behavior: 'smooth' });
  };
  ...
  return (
    <div className="flex flex-col bg-gray-50 pb-24">
      ...
      <div className="relative overflow-hidden hero-water w-full shadow-inner z-0 px-6 pt-10 pb-14 text-center flex flex-col items-center">
          ...
          <img src="/logo-final.png" alt="logo" className="w-32 h-32 object-contain animate-float-gen drop-shadow-2xl mb-6" />
          <h1 className="text-white font-black text-4xl leading-none tracking-tighter uppercase italic">El Pollazo</h1>
          <h2 className="font-black text-2xl leading-none text-yellow-300 tracking-widest uppercase">El Mirador</h2>
      </div>
        ...
        <RevealOnScroll>
          <div className="bg-white/90 backdrop-blur-xl border-2 border-white rounded-[35px] p-6 shadow-xl flex items-center justify-between">
            <div className="flex items-center gap-4 text-left">
              <div className="w-12 h-12 bg-gradient-to-tr from-orange-400 to-orange-600 rounded-2xl flex items-center justify-center text-white rotate-3">
                <Zap size={24} fill="currentColor" />
              </div>
              <div className="leading-none">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 italic">Hola, {user?.customerName || 'Guerrero'}</p>
                <div className="flex items-center gap-1.5">
                  <span className="text-2xl font-black text-gray-900">{(user?.points || 0).toLocaleString()}</span>
                  <span className="text-orange-500 font-black text-[9px] uppercase mt-1">Pollazo Puntos 🍗</span>
                </div>
              </div>
            </div>
            <button onClick={() => onNavigate('ranking')} className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-orange-500"><ChevronRight size={20} /></button>
          </div>
        </RevealOnScroll>
      </div>
      ...
      <div className="px-6 py-8 grid grid-cols-2 gap-4">
          <button className="w-full bg-slate-900 rounded-[30px] p-5 text-left relative overflow-hidden group shadow-xl border-b-4 border-black active:scale-95 transition-all">
          ...
          <button className="w-full bg-white border border-gray-200 rounded-[30px] p-5 text-left relative overflow-hidden group shadow-sm border-b-4 active:scale-95 transition-all">
