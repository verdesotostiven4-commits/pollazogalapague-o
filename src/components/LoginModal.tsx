    ...
    <div className="fixed inset-0 z- flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white rounded-[36px] p-6 shadow-2xl relative animate-in zoom-in-95 duration-300">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          {AVATARS.map((avatar, idx) => (
            <button 
              ...
              className={`relative aspect-square rounded-2xl overflow-hidden border-2 transition-all ${selectedAvatar === avatar && !uploadedImage ? 'border-orange-500 scale-105 shadow-lg' : 'border-transparent'}`}
            >
        ...
        <button 
          type="button"
          onClick={handleSave} 
          className="mt-8 w-full py-4 bg-orange-500 text-white font-black rounded-[24px] shadow-xl shadow-orange-200 active:scale-95 transition-all"
        >
          GUARDAR CAMBIOS
        </button>
      </div>
