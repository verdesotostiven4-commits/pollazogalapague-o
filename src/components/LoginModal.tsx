...
const AVATARS = [
  ...
  // ✅ AVATAR #7 AGREGADO (Temática aventurero)
...
export default function LoginModal({ isOpen, onClose, onLogin }: LoginModalProps) {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files ? e.target.files : null; // ✅ Fix: Acceder al primer archivo
    if (file) {
  ...
  const handleSave = () => {
    ...
    // ✅ Ahora sí se guarda y se cierra
    onLogin({ name, whatsapp, avatarUrl: selectedAvatar });
    onClose(); 
  };

  return (
    /* ✅ Z-INDEX CORREGIDO A 100 PARA ESTAR POR ENCIMA DE TODO */
