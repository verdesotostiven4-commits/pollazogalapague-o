...
export default function OrderTracking() {
  ...
  const order = myOrders; // Tomamos el más reciente
  ...
  return (
    <div className="fixed bottom-24 right-4 z-40 flex flex-col items-end pointer-events-none">
      {isOpen && (
        <div className="mb-3 w-[85vw] max-w-[340px] bg-white rounded-[32px] p-6 shadow-2xl border-2 border-orange-500 animate-in fade-in slide-in-from-bottom-4 duration-300 pointer-events-auto">
          <div className="flex justify-between items-start mb-4">
            <div>
              <span className="bg-orange-100 text-orange-600 text-[10px] font-black px-2 py-1 rounded-lg uppercase">Estado de Compra</span>
              <p className="text-lg font-black text-gray-900 mt-1">Orden #{order.order_code}</p>
            </div>
            <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-gray-100 rounded-full"><X size={18} /></button>
          </div>
            ...
            {statusSteps.map((step, idx) => {
              ...
              return (
                <div key={step.status} className="flex flex-col items-center gap-2 z-10">
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${isCompleted ? 'bg-orange-500 text-white shadow-lg' : 'bg-white border-2 border-gray-100 text-gray-300'}`}>
                    <Icon size={18} className={order.status === step.status && step.status === 'Preparando' ? 'animate-bounce' : ''} />
                  </div>
                  <span className={`text-[8px] font-black uppercase ${isCompleted ? 'text-gray-900' : 'text-gray-300'}`}>{step.label}</span>
                </div>
              );
            })}
          </div>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="pointer-events-auto flex items-center gap-3 bg-white text-orange-500 px-4 py-3 rounded-2xl shadow-xl border-2 border-orange-500 transition-all active:scale-95">
        ...
        <div className="flex flex-col items-start"><span className="text-[9px] font-black text-gray-400 uppercase">Tu pedido</span><span className="text-[12px] font-black uppercase">{order.status}</span></div>
