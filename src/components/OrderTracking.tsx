    ...
    <div className="fixed inset-0 z- flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-[40px] p-8 shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-black text-gray-900 uppercase italic leading-none">{isTrackingNow ? 'Estado del Pedido' : 'Rastreo en Vivo'}</h2>
          <p className="text-sm font-bold text-gray-400 mt-2">{isTrackingNow ? `Código: #${activeOrder.order_code}` : 'Sigue tu compra paso a paso'}</p>
        </div>
        {isTrackingNow ? (
          ...
          <div className="space-y-6 text-center">
            <p className="text-sm text-gray-500 font-bold leading-relaxed px-2">Aquí podrás ver el progreso de tu compra en tiempo real. Cuando realices un pedido y lo confirmemos, se activará este seguimiento. 🛵💨</p>
            <div className="p-5 bg-blue-50 rounded-[32px] flex items-center gap-4 border border-blue-100">
              ...
              <p className="text-[10px] font-black text-blue-700 uppercase leading-tight text-left">Te notificaremos cuando el pedido salga de "La Casa del Pollazo".</p>
            </div>
            <button onClick={onClose} className="w-full py-5 bg-gray-900 text-white font-black rounded-[24px] text-xs uppercase tracking-widest active:scale-95 transition-all">¡Entendido!</button>
          </div>
        )}
      </div>
    </div>
