...
type SupportedPaymentMethod = Extract<
  PaymentMethod,
  'efectivo' | 'deuna' | 'transferencia'
>;
...
export default function CartScreen({
  ...
  const handlePaymentMethodClick = (method: SupportedPaymentMethod) => {
    ...
    if (isOrderSaved && paymentMethod && method !== paymentMethod) {
      triggerDryTap();
      showNotice(
        'El método de pago ya quedó asociado a este pedido. Finaliza o crea un pedido nuevo.'
      );
      return;
    }
  ...
  const persistPaymentSelection = () => {
    ...
    if (paymentMethod === 'transferencia') {
      ...
      localStorage.setItem('selectedBank', 'Ninguno');
    }
  };
  ...
  const validateBeforeSave = () => {
    ...
    if (!isPaymentReady) {
      triggerDryTap();
      showNotice('Revisa el método de pago antes de continuar.');
      return false;
    }
  ...
  const saveOrderNow = async () => {
    if (isSavingOrder || isOrderSaved) return isOrderSaved;
    ...
    try {
      ...
      if (paymentMethod === 'efectivo') {
        showNotice('Pedido registrado. Ahora puedes confirmar dentro de la app.');
      } else {
        showNotice('Pedido registrado. Revisa los datos de pago y luego confirma.');
      }
  ...
  const handleRegisterOrderClick = async () => {
    const saved = await saveOrderNow();

    if (!saved) return;

    if (paymentMethod === 'efectivo') {
      spawnConfetti();
      window.setTimeout(() => onCheckout(), 220);
    } else {
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 flex gap-3 animate-in fade-in duration-300">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-amber-700/80 leading-relaxed mt-1">
              Presiona “Registrar pedido”. Luego podrás ver la confirmación dentro de la app.
            </p>
          </div>
        </div>
      );
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-2xl p-3 flex gap-3 animate-in fade-in duration-300">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-orange-700/80 leading-relaxed mt-1">
              Tu pedido quedó registrado. El negocio revisará disponibilidad y luego verás el avance en el rastreo.
            </p>
          </div>
        </div>
      );
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-3 flex gap-3 animate-in fade-in duration-300">
        ...
        <div>
          ...
          <p className="text-[10px] font-bold text-blue-700/80 leading-relaxed mt-1">
            El pedido ya está guardado. Realiza el pago y luego confirma; por ahora el comprobante se puede enviar desde la pantalla final.
          </p>
        </div>
      </div>
    );
  ...
  const primaryButtonText = () => {
    if (!hasProfile || !hasLocation) return 'Completar datos para entregar';
    if (!paymentMethod) return 'Selecciona un método de pago';
    if (isSavingOrder) return 'Registrando pedido...';
    ...
    if (paymentMethod === 'efectivo') {
      return 'Ver confirmación del pedido';
    }
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden relative">
      ...
      >
        ...
        {items.map(item => {
          ...
          const itemSubtotal =
            unitPrice > 0 ? (unitPrice * item.quantity).toFixed(2) : null;
        ...
        <div className="pt-4 border-t border-gray-100 space-y-3">
            ...
            <div className="bg-yellow-50 border border-yellow-100 rounded-2xl p-3 flex gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-yellow-700/80 leading-relaxed mt-1">
                  Deuna y transferencia se activan solo cuando todos los productos tienen precio exacto. Puedes dejar el pedido por confirmar.
                </p>
              </div>
            </div>
          )}
                ...
                {BANK_OPTIONS.map(bank => (
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                      ...
                      isOrderSaved && selectedBank !== bank.id
          ...
          {isOrderSaved && paymentMethod === 'transferencia' && selectedBank && (
            <div className="bg-blue-50/40 rounded-2xl p-3 border border-blue-100 space-y-2 mt-2 animate-in fade-in duration-300">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="flex justify-between items-center bg-white p-2 rounded-xl border border-blue-50">
                  ...
                  <button
                    type="button"
                    onClick={() => handleCopyText(BUSINESS_BENEFICIARY, 'beneficiario')}
                    className="text-[10px] bg-blue-100 text-blue-700 font-bold px-2 py-1 rounded-lg active:scale-90 transition-all"
                  >
                    {copiedLabel === 'beneficiario' ? '¡Copiado!' : 'Copiar'}
                  </button>
                </div>
              </div>

              <p className="text-[10px] text-blue-500 font-black uppercase tracking-tight text-center pt-1">
                Luego toca “Ya pagué / continuar”.
              </p>
            </div>
          )}
          ...
          {isOrderSaved && paymentMethod !== 'efectivo' && (
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 flex gap-3">
              <div className="w-9 h-9 bg-white rounded-xl flex items-center justify-center text-slate-500 shadow-sm flex-shrink-0">
                <Clipboard size={17} />
              </div>
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-500 leading-relaxed mt-1">
                  En la pantalla final podrás escribir por WhatsApp para enviar el comprobante. Luego agregaremos carga de comprobante dentro de la app.
                </p>
              </div>
            </div>
          )}
      ...
      {showArrow && (
        <button
          ...
          className="absolute right-4 bottom-[118px] z-10 bg-white text-orange-500 border border-orange-100 shadow-xl rounded-full w-10 h-10 flex items-center justify-center active:scale-90 transition-transform"
      ...
      <div className="border-t border-gray-100 bg-white px-4 pt-3 pb-[calc(env(safe-area-inset-bottom)+14px)] shadow-[0_-10px_30px_rgba(15,23,42,0.08)]">
        ...
        <button
          ...
          className={`w-full h-14 rounded-[24px] font-black uppercase tracking-widest text-[12px] flex items-center justify-center gap-2 active:scale-95 transition-all shadow-xl ${
            primaryButtonDisabled()
              ? 'bg-gray-200 text-gray-400 cursor-not-allowed shadow-none'
              : isOrderSaved
                ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-green-500/30 border-b-4 border-emerald-700'
                : 'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-orange-500/35 border-b-4 border-orange-700'
          }`}
        >
          {isSavingOrder ? (
            <>
        ...
        {isOrderSaved && (
          <p className="text-center text-[9px] font-black text-gray-400 uppercase mt-2 leading-relaxed">
            El pedido ya está guardado. El siguiente paso no te saca de la app.
          </p>
        )}
      </div>
